...
/**
* This class Handles Approve and Reject quote on backend
* add shortcode on selected approval rejection page.
...
class QuoteupHandleQuoteApprovalRejection
{
     ...
     * Action to add shortcode on selected approval rejection page.
     * Show notices on dashboard if page not selected or some other error
     */
    ...
    public function getNoticePageNotSelected()
    {
        if ($_SERVER['REQUEST_URI'] != '/wp-admin/admin.php?page=quoteup-for-woocommerce') {
            ...
            $html .= '<p>';
            $html .= sprintf(__('Please set a page where users can approve or reject the quote %s on this page %s', QUOTEUP_TEXT_DOMAIN), "<a href='admin.php?page=quoteup-for-woocommerce#wdm_quote'>", '</a>');
            $html .= '</p>';
            $html .= '</div><!-- /.error -->';
     ...
     * @param  object $selectedPage page for approval rejection
     * @param  int $pageId       page Id for approval rejection page
     */
    public function shortcodeCheck($selectedPage, $pageId)
    {
        if (quoteupDoesContentHaveShortcode($selectedPage->post_content, 'APPROVAL_REJECTION_CHOICE') === false) {
            $html = '<div class="error">';
            $html .= '<p>';
            $html .= sprintf(__('Page you have set for approval and rejection of quote does not have the shortcode [APPROVAL_REJECTION_CHOICE]. Please add [APPROVAL_REJECTION_CHOICE] in the content of that page  %s here %s.', QUOTEUP_TEXT_DOMAIN), "<a href='post.php?post={$pageId}&action=edit'>", '</a>');
            $html .= '</p>';
            $html .= '</div><!-- /.error -->';
            echo $html;
        }
    ...
    // Approval/Rejection page is not set in the Settings
        ...
        // Check page status and show warning if page's status is Trash
                if ($selectedPage->post_status == 'trash') {
                    $html = '<div class="error">';
                    $html .= '<p>';
                    $html .= sprintf(__('Page you have set for approval and rejection of quote is in Trash. Therefore, users won\'t be able to approve/reject quotes. You can change the status of that page %s here %s.', QUOTEUP_TEXT_DOMAIN), "<a href='edit.php?post_status=trash&post_type=page'>", '</a>');
                    $html .= '</p>';
                    $html .= '</div><!-- /.error -->';
                    echo $html;
                }
                ...
                $html = '<div class="error">';
                $html .= '<p>';
                $html .= sprintf(__('Page you have set for approval and rejection of quote does not exist. Please set a new page for approval/rejection of quotes  %s here %s.', QUOTEUP_TEXT_DOMAIN), '<a href="admin.php?page=quoteup-for-woocommerce#wdm-approval-rejection-page">', '</a>');
                $html .= '</p>';
                $html .= '</div><!-- /.error -->';
                echo $html;
            }
    ...
    {
        $quoteup_lic_stat = get_option('edd_pep_license_status');
        if ($_SERVER['REQUEST_URI'] != '/wp-admin/plugins.php?page=wisdmlabs-licenses' && $quoteup_lic_stat != 'valid' && $quoteup_lic_stat != 'expired') {
     ...
     /**
     * This function checks if enquiries are migrated or not, if not it displays a notice to migrate enquiry.
     */
    public function migrationNotice()
    {
        if ($_SERVER['REQUEST_URI'] != '/wp-admin/admin.php?page=quoteup-for-woocommerce') {
            ...
            if ($wpdb->get_var("SHOW TABLES LIKE '$enquiry_tbl'") == $enquiry_tbl && $migrated != 1) {
                $html = '<div class="error">';
                $html .= '<p>';
                $html .= sprintf(__('It seems you have upgraded from Product Enquiry Pro 2.x to QuoteUp. There have been database architectural changes in QuoteUp. We strongly recommend to migrate the previous enquiries to QuoteUp %s here %s before you start using Quoteup.', QUOTEUP_TEXT_DOMAIN), '<a href="admin.php?page=quoteup-for-woocommerce">', '</a>');
                $html .= '</p>';
                $html .= '</div><!-- /.error -->';
    ...
    public function addShortcodeOnPageOnUpdateOption($hookName, $oldValue, $newValue)
    {
        ...
        if (!isset($oldValue['approval_rejection_page']) || !isset($newValue['approval_rejection_page']) || $oldValue['approval_rejection_page'] != $newValue['approval_rejection_page']) {
     ...
     *
     * @param string $hookName Name of the hook
     * @param string $newValue Settings data
     */
