...
class Date
{
            ...
            <div class="cog" id="cog_<?php echo $fieldindex; ?>" style='display: none'>
                <fieldset>
                    ...
                    <div class='form-group'>
                        <label><?php _e('Min Date', QUOTEUP_TEXT_DOMAIN) ?>:</label>
                        <input class='form-control datepicker row-bottom-buffer' type="text" value="<?php echo $field_infos[$fieldindex]['min_date'] ?>" name="contact[fieldsinfo][<?php echo $fieldindex ?>][min_date]"/>                        
                    </div>
                    <div class='form-group'>
                        <label><?php _e('Max Date', QUOTEUP_TEXT_DOMAIN) ?>:</label>
                        <input class='form-control datepicker row-bottom-buffer' type="text" value="<?php echo $field_infos[$fieldindex]['max_date'] ?>" name="contact[fieldsinfo][<?php echo $fieldindex ?>][max_date]"/>                        
                    </div>
    ...
    public function field_render_html($params = array())
    {
        ...
        $min_date = !isset($params['min_date']) ? "" : $params['min_date'];
        $max_date = !isset($params['max_date']) ? "" : $params['max_date'];
        $datepicker_field_id = "datepicker_{$params['id']}".rand();
        $condition_fields = '';
        $cond_action = '';
        $cond_boolean = '';
    ...
    <script type="text/x-mustache" id="template-Date">
        <li class="list-group-item" data-type="<?php echo str_replace("Wisdmforms\\", "", __CLASS__) ?>" id="field_{{ID}}"><input type="hidden" name="contact[fields][{{ID}}]" value="{{value}}">
            ...
            <div class="cog" id="cog_{{ID}}" style="display: none;">
                ...
                <div class='form-group'>
                    <label><?php _e('Min Date', QUOTEUP_TEXT_DOMAIN) ?>:</label>
                    <input class='form-control datepicker row-bottom-buffer' type="text" value="" name="contact[fieldsinfo][{{ID}}][min_date]"/>
                </div>
                <div class='form-group'>
                    <label><?php _e('Max Date', QUOTEUP_TEXT_DOMAIN) ?>:</label>
                    <input class='form-control datepicker row-bottom-buffer' type="text" value="" name="contact[fieldsinfo][{{ID}}][max_date]"/>
                </div>
