...
class Textarea
{
    public function control_button()
    {
        ob_start();
        ?>
        <li class="list-group-item" data-type="<?php echo str_replace("Wisdmforms\\","",__CLASS__) ?>" for="Textarea">
            <span class="lfi lfi-name"></span> <?php _e('Textarea', QUOTEUP_TEXT_DOMAIN) ?> 
            <a title="<?php _e('Textarea', QUOTEUP_TEXT_DOMAIN) ?>" rel="Textarea" class="add" data-template='Textarea' href="#"><i class="glyphicon glyphicon-plus-sign pull-right ttipf" title=""></i></a>
        </li>
        <?php
        // control_button_html
        return ob_get_clean();
    ...
    public function field_render_html($params = array())
    {
        ...
        <div id="<?php echo $params['id'] ?>" class='form-group <?php echo isset($params['conditioned']) ? " conditioned hide " : ''?>' data-cond-fields="<?php echo $condition_fields ?>" data-cond-action="<?php echo $cond_action.':'.$cond_boolean ?>" >
            ...
            ?>
            <textarea  name='submitform[<?php echo $name ?>]' cols='1' rows='1' class='form-control' placeholder='<?php echo esc_attr(quoteupReturnCustomFormFieldPlaceholder($params['label'])); ?>'  <?php echo required($params); ?>  ></textarea>
            <div>
    ...
    public function configuration_template()
    {
        ob_start();
        ?>
    <script type="text/x-mustache" id="template-Textarea">
