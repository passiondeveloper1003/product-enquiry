...
class Number
{
    public function control_button()
    {
        ob_start();
        ?>
        <li class="list-group-item" data-type="<?php echo str_replace("Wisdmforms\\","",__CLASS__) ?>" for="Number">
            <span class="lfi lfi-name"></span> <?php _e('Number', QUOTEUP_TEXT_DOMAIN) ?> 
            <a title="<?php _e('Number', QUOTEUP_TEXT_DOMAIN) ?>" rel="Number" class="add" data-template='Number' href="#"><i class="glyphicon glyphicon-plus-sign pull-right ttipf" title=""></i></a>
        </li>
        <?php
        // control_button_html
        return ob_get_clean();
...
?>
    ...
    public function configuration_template()
    {
        ob_start();
        ?>
    <script type="text/x-mustache" id="template-Number">
