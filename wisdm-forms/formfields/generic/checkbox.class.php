...
class Checkbox
{
    public function control_button()
    {
        ob_start();
        ?>
        <li class="list-group-item" data-type="<?php echo str_replace("Wisdmforms\\","",__CLASS__) ?>" for="Checkbox">
            <span class="lfi lfi-name"></span> <?php _e('Checkbox', QUOTEUP_TEXT_DOMAIN) ?> 
            <a title="<?php _e('Checkbox', QUOTEUP_TEXT_DOMAIN) ?>" rel="Checkbox" class="add" data-template='Checkbox' href="#"><i class="glyphicon glyphicon-plus-sign pull-right ttipf" title=""></i></a>
        </li>
        <?php
        // control_button_html
        return ob_get_clean();
...
?>
    ...
    public function field_preview_html($params = array())
    {
        ...
        ?>
            <label><input type='checkbox' disabled="disabled" value='<?php echo $option_values[$id] ?>' name='submitform[<?php echo $params['id'] ?>][]' <?php echo required($params) ?>   /><?php echo $label ?></label>
        <?php
        }
        ?>
        </div>
    ...
    public function field_render_html($params = array())
    {
        ...
        <div id="<?php echo $params['id'] ?>" class='form-group <?php echo isset($params['conditioned']) ? " conditioned hide " : ''?>' data-cond-fields="<?php echo $condition_fields ?>" data-cond-action="<?php echo $cond_action.':'.$cond_boolean ?>" >
            ...
            foreach ($option_names as $id => $label) {
            ?>
                <label><input type='checkbox' value='<?php echo $option_values[$id] ?>' name='submitform[<?php echo $name; ?>][]' <?php echo required($params) ?> /><?php echo quoteupReturnCustomFormFieldLabel($label); ?></label>
            <?php
            }
            ?>
            </div>
