...
class Select
{
    public function control_button()
    {
        ob_start();
        ?>
        <li class="list-group-item" data-type="<?php echo str_replace("Wisdmforms\\","",__CLASS__) ?>" for="Select">
            <span class="lfi lfi-name"></span> <?php _e('Select', QUOTEUP_TEXT_DOMAIN) ?> 
            <a title="<?php _e('Select', QUOTEUP_TEXT_DOMAIN) ?>" rel="Select" class="add" data-template='Select' href="#"><i class="glyphicon glyphicon-plus-sign pull-right ttipf" title=""></i></a>
        </li>
        <?php
        // control_button_html
        return ob_get_clean();
...
?>
    ...
    public function field_preview_html($params = array())
    {
        ...
        ob_start();
        ?>
        <select disabled="disabled" name='submitform[]' <?php echo isset($params['multi'])?'multiple':'' ?> <?php echo required($params) ?>>
    ...
    public function field_render_html($params = array())
    {
        ...
        <div id="<?php echo $params['id'] ?>" class='form-group <?php echo isset($params['conditioned']) ? " conditioned hide " : ''?>' data-cond-fields="<?php echo $condition_fields ?>" data-cond-action="<?php echo $cond_action.':'.$cond_boolean ?>" >
            ...
            <?php
            foreach ($option_names as $id => $label) { ?>
                <option value='<?php echo $option_values[$id] ?>'><?php echo quoteupReturnCustomFormFieldLabel($label); ?></option>
            <?php
            }
            ?>
            </select>
    ...
    public function configuration_template()
    {
        ob_start();
        ?>
    <script type="text/x-mustache" id="template-Select">
