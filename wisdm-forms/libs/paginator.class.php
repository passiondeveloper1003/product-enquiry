...
class Paginator {
    ...
    function generate_page_links_html() {
        ...
        $begin_list_html = '';
        if ($starting_page > 1) {
            $begin_list_html = ('<a '.'href="'.$this->fixed_url.'&from_page='.$current_page."&to_page=".'1'.'"'.' class="'.$this->styles['normal'].' '.($this->styles['active'].'"').'>'.'1'.'</a>'."\n");
        }
        if ($starting_page > 2) {
            $begin_list_html .= ('<a '.'href="'.$this->fixed_url.'&from_page='.$current_page."&to_page=".'2'.'"'.' class="'.$this->styles['normal'].' '.($this->styles['active'].'"').'>'.'2'.'</a>'."\n");
        }
        if ($begin_list_html != '') {
            $begin_list_html .= '  . . .  ';
        }
        ...
        $end_list_html = '';
        if ($ending_page < $this->page_count-1) {
            $end_list_html = ('<a '.'href="'.$this->fixed_url.'&from_page='.$current_page."&to_page=".($this->page_count-1).'"'.' class="'.$this->styles['normal'].' '.($this->styles['active'].'"').'>'.($this->page_count-1).'</a>'."\n");
        }
        if ($ending_page < $this->page_count) {
            $end_list_html .= ('<a '.'href="'.$this->fixed_url.'&from_page='.$current_page."&to_page=".($this->page_count).'"'.' class="'.$this->styles['normal'].' '.($this->styles['active'].'"').'>'.($this->page_count).'</a>'."\n");
        }
        if ($end_list_html != '') {
            $end_list_html = '  . . .  '.$end_list_html;
        }
