                ...
                <div class="panel-body">
                            <table class="table">
                                <?php foreach ($form_fields as $field_id => $field_pref) {
                                    if (strpos($field_id, 'Captcha_') !== false) {
                                        continue;
                                    }
                                    ?>
                                    <?php if (!in_array(substr($field_id, 0, strpos($field_id, '_')), $non_submit_fields)) { ?>
                                    <tr>
                                        <th><?php echo esc_attr($field_pref['label']) ?></th>
                                        <td><?php echo is_array($field_values[$field_id]) ? esc_attr(implode(', ', $field_values[$field_id])) : esc_attr($field_values[$field_id]) ?></td>
                                    </tr>
                                    <?php } ?>
                                <?php } ?>
                            </table>                    
